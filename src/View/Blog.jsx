...
function Blog() {
    const [currentPage, pageSizeSelected, onPageChange, onPageSizeChange] = usePagination(10);
    ...
    const handleBlogSearch = (event) => {
        ...
        if (event.target.value) {
            ...
            setFilteredBlogList(filteredBlog);
        } else {
    ...
    //useEffect First time DOM load hota h tb call hota h
    //and uske bad []array me jo bhi event ya function pass hoga uske call hone pr run hoga useEffect
    useEffect(() => {
    ...
    return (
        <>
            <h2 className="mt-6 text-center text-3xl text-amber-500">
                Blog listing using 
                <strong className="font-semibold"> useEffect</strong>
            ...
            <div className="flex items-center justify-center h-screen">
                <div className="animate-spin rounded-full border-t-4 border-blue-500 border-solid border-9 h-12 w-12">
                </div>
            </div>
            )}
